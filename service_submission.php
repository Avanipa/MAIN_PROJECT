...
$sql = "select * from tbl_services where serviceid='$service' and status=1";
$result = mysqli_query($conn, $sql);
  while($row=mysqli_fetch_array($result)){
    $description = $row['service_description'];
    $eligibility = $row['criteria'];
    $age = $row['category'];
    $service_name = ucfirst($row['subservice']);
...
// Checking whether fid exisyt for document or not    
    ...
    if($flag==0){
      ...
      $sql = "select * from tbl_user_services where service_id='$service'";
      $result = mysqli_query($conn, $sql);
      while($row = mysqli_fetch_assoc($result)){
        $serviceExist = 1;
      }
...
// ***************************************Move Uploaded File Function End********************************************************* 
                ...
                $rejection = 0;
                $scheme_insert = "insert into tbl_user_services(doc_id, reg_id, service_id, status, comments, applied_date, rejection_status, approval_type) values
                ((select doc_id from tbl_documents where fid='$reg_id'), '$reg_id', '$service', 1, '$comments', '$applied_date', '$rejection', 0)";
...
<body>
    ...
    <div class="container">
      <h4><b>Service Name : </b><?php echo $service_name; ?></h4><br>
