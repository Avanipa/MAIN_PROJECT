...
if(isset($_POST['submit'])){
    ...
    $sql = "update tbl_qualification set quali_reg_id='$quali_reg_id', qualification='$qualification', institution='$institution', board='$board',
    register_no='$register_no', percentage='$percentage', cgpa='$cgpa', passoutmonth='$month', passoutyear='$year' where qualification_id='$qualfication_id'";
...
if(isset($_GET['id'])){
    $dis_id = $_GET['id'];
    $dis_sql = "select * from tbl_qualification where qualification_id='$dis_id'";
    $dis_result = mysqli_query($conn, $dis_sql);
    $dis_row = mysqli_fetch_assoc($dis_result);
    $month = $dis_row['passoutmonth'];
    $year = $dis_row['passoutyear'];
    $passouttime = $year.'-'.$month;
                ...
                <center>
                    <form method="POST" id="qual_form" style="width: 80%;">
                        <select class="form-select" name="qualification" id="qualification">
                            <option value=<?php echo $dis_row['qualification'] ?>><?php echo $dis_row['qualification'] ?></option>
                        ...
                        <input type="text" name="institution" value="<?php echo $dis_row['institution'] ?>" placeholder="Enter your institution" required><br>
                        
                        <select class="form-select" name="board" id="board">
                            <option value=<?php echo $dis_row['board'] ?>><?php echo $dis_row['board'] ?></option>
                        ...
                        <input type="number" name="register_no" value=<?php echo $dis_row['register_no'] ?> placeholder="Enter your register number" required>
                        <div style="display: flex; justify-content: space-between;">
                            <input type="number" name="percentage" id='percentage' value=<?php echo $dis_row['percentage'] ?> placeholder="Enter your percentage"> OR
                            <input type="number" name="cgpa" id='cgpa' value=<?php echo $dis_row['cgpa'] ?> placeholder="Enter your cgpa">>
                        </div>
                        ...
                        <h5 style="float: left; font-size: 18px; padding-left: 10px;">Please select the passout date: <input type="month" name="passout" value=<?php echo $passouttime ?> required></h5><br><br><br>
