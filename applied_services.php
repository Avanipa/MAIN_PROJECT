...
$sql = "select * from tbl_user_services where reg_id='$id'";
...
if(isset($_POST['re_submit'])){
    ...
    $approval = "update tbl_user_services set status=1, comments='$comment', approval_type=0 where service_id='$reject_id'";
    if(mysqli_query($conn, $approval)){
        echo "<script>";
            echo "alert('Service has been re submitted')";
        echo "</script>";
    }else{
        mysqli_error($conn);
    }
                ...
                <center>
                    <table class="account_table">
                        <tr style="background-color: antiquewhite">
                            <th style="width: 400px; text-align: left;padding: 10px;">Applied Services</th>
                        ...
                        <?php
                            for($j=0; $j<count($service_id); $j++){
                                echo "<tr style='box-shadow: 0px 5px 8px 1px black'>";
                                    echo "<td>";
                                        $sql = "select * from tbl_services where serviceid='$service_id[$j]'";
                                        $result = mysqli_query($conn, $sql);
                                        while($row = mysqli_fetch_assoc($result)){
                                            echo $row['subservice'];
                                        }
                                        $status = $statuses[$j];
                                        if($status==100){
                                            echo "<h5 style='font-size:12px; color:red'>"."Please reupload the rejected document in "."<a href='./mydocuments'>"."<u>"."My Documents"."</u>"."</a>"." tab, Once completed please click on the below button"."</h5>";
                                            echo "<form method='POST'>";
                                                echo "<input type='hidden' name='rejection_id' value='$service_id[$j]'/>"; 
